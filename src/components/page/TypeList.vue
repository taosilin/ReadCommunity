  ...
  <div>
    ...
    <div class="booklist" v-for="(item, index) in booklist" @click="onBookDetail(index)">
      <el-row>
        ...
        <el-col :span="12">
          <el-row><div class="book-name">{{item.book_name}}</div></el-row>
          <el-row><div class="author">{{item.author}}/{{item.publisher}}</div></el-row>
...
export default {
  ...
  created () {
    axios.post('http://106.13.75.89:8080/book/type', {'tag': this.$route.params.type, 'page': 0, 'size': 10})
      .then((response) => {
  ...
  methods: {
    ...
    handleSizeChange (val) {
      ...
      axios.post('http://106.13.75.89:8080/book/type', {'tag': this.$route.params.type, 'page': this.page, 'size': this.size})
        .then((response) => {
          console.log(response.data.data)
          this.booklist = response.data.data
        }).catch((error) => {
